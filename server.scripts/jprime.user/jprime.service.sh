...
#        pushd .
       	cd $SOURCE_ROOT
#      	git pull origin master -v --dry-run 2>&1 | grep "up to date" && changed=0 || changed=1; if [ $changed -eq 0 ]; then echo NOTHING CHANGED, WILL FAIL THE BUILD; exit 1; else echo CHANGES F$
       	git pull origin master
        /usr/bin/mvn clean package -P \!run.as.spring-boot.run -DskipTests=true
#       popd #this changes the $? FIXME TODO
        ;;
	...
	log)
		...
		;;
	less)
		less $TOMCAT_HOME/logs/catalina.out
		;;
	kill)
	...
	*)
        ...
        echo -e "\tUSAGE: $0 rebuild/redeploy/log/start/stop/restart/manual/kill/less"
        echo -e "\tNOTES:"
        echo -e "\t\t'rebuild' will git pull, and mvn clean package with a custom profile, but only if changes found, otherwise nothing will be done"
        echo -e "\t\t'force-rebuild' will git pull, and mvn clean package with a custom profile even if no changes found"

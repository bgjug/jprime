...
public class AgendaController {
    ...
    public String getAgenda(Model model) {
        ...
        //option one iterate on all alpha on the view and with status=i and use i.count in beta to get the
		// beta talk with the same position as in alpha.
        //pros - easy, cons - not good when we have a missing slots on beta or when slots dont match !

        //option two is to create a structure with slots (indexes or timings 09:00-09:40 and so on .. and
		// to put on each slot the one(when break) or two (when talks for alpha and beta)
